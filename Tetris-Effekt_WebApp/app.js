...
// Spiegel-Geschwister (chiral): sehen ähnlich aus, passen aber nie in die Lücke des anderen
const MIRROR = { S:'Z', Z:'S', L:'J', J:'L' };
// Rotationsindizes, die optisch UNTERSCHIEDLICHE Lagen ergeben (O hat nur 1, I/S/Z 2, T/L/J 4)
function distinctRots(key){
  const seen = {}, res = [];
  for(let r=0;r<4;r++){ const k = cellKey(rotateN(SHAPES[key].cells, r)); if(!(k in seen)){ seen[k]=1; res.push(r); } }
  return res;
}
...
function buildGapfitTrials(){
  const keys = Object.keys(SHAPES);
  const targets = keys.filter(k=>k!=='O');   // O hat nur eine Lage -> als Ziel keine Rotation nötig
  const trials = [];
  for(let i=0;i<CFG.GAPFIT_TRIALS;i++){
    const correctKey = pick(targets);
    const drs = distinctRots(correctKey);
    const gapRot = pick(drs);                                   // Orientierung der Lücke
    const gapNorm = cellKey(rotateN(SHAPES[correctKey].cells, gapRot));
    // Korrektes Teil in ANDERER sichtbarer Lage als die Lücke -> muss gedanklich gedreht werden
    const turned = drs.filter(r => cellKey(rotateN(SHAPES[correctKey].cells, r)) !== gapNorm);
    const dispRot = turned.length ? pick(turned) : gapRot;
    const correct = { key:correctKey, cells:rotateN(SHAPES[correctKey].cells, dispRot), color:SHAPES[correctKey].color, isCorrect:true };
    // Distraktoren: andere Tetromino-TYPEN (passen unter keiner Drehung). Ab der Hälfte: Spiegel-Geschwister erzwingen.
    const distractKeys = [];
    if(i >= CFG.GAPFIT_TRIALS/2 && MIRROR[correctKey]) distractKeys.push(MIRROR[correctKey]);
    for(const k of shuffle(keys.filter(k=>k!==correctKey))){ if(distractKeys.length>=3) break; if(!distractKeys.includes(k)) distractKeys.push(k); }
    const distract = distractKeys.slice(0,3).map(k=>({ key:k, cells:rotateN(SHAPES[k].cells, pick(distinctRots(k))), color:SHAPES[k].color, isCorrect:false }));
    const options = shuffle([correct, ...distract]);
    trials.push({ correctKey, gapRot, dispRot, gapCells: placeGap(correctKey, gapRot), options, correctIndex: options.findIndex(o=>o.isCorrect) });
  }
  return trials;
}
async function runGapfit(){
  const trials = buildGapfitTrials();
  const results = [];
  ...
  for(let i=0;i<trials.length;i++){
    ...
    results.push({ i:i+1, shape:t.correctKey, gapRot:t.gapRot, dispRot:t.dispRot, chosen:Number(ans), correctIndex:t.correctIndex, correct, rt:round(rt) });
    await feedbackFlash(host, correct);
  }
  const rts = results.filter(r=>r.correct).map(r=>r.rt);
